...
- **The Video Studio needs an https address.** It encodes the video in your browser, using an interface browsers only expose on a secure page, so over a plain `http://` address — your Home Assistant IP, or `homeassistant.local` — it stops with "This browser cannot encode video", whichever browser you use. Through Nabu Casa or your own https proxy, rendering, saving to the gallery and downloading all work. Saved videos live in `/data` and are backed up with the add-on: the 10 newest per account are kept, files older than 30 days are removed
- **Slow first boot — give the loading page time and do not restart the add-on while it is up.** A new index over your points is built during the upgrade, and every history is re-checked afterwards in the background, so expect high CPU for a while. Legs faster than 1200 km/h no longer count towards distance, so past monthly totals shift, typically by well under a percent. The background worker now waits for the database work to finish, so a busy disk can no longer leave the app restarting in a loop. Your points, trips and imports are untouched
- **Time in a city is measured by how long you stayed, not how often your phone reported.** Recalculated months roughly double the time in the places you actually spent time, while brief drive-throughs and duplicate spellings of the same place drop out — expect shorter city lists with honest numbers. **Existing months repair themselves** in the background instead of waiting until you press "Update stats", and countries the geocoder spells differently are repaired the same way. The "Max Gap Between Points" map setting is retired
- Import fixes for GPX files without elevation, Polarsteps exports and Google Timeline altitudes; Hebrew and Arabic map labels now read right to left. No app config changes required
