...
- **The Video Studio needs an https address.** It encodes in your browser, using an interface browsers only expose on a secure page, so an unencrypted address such as `http://homeassistant.local:8123` will not offer it. Through Nabu Casa or your own https proxy it works. Saved videos live in `/data` and are backed up with the add-on: the 10 newest per account are kept, files older than 30 days are removed
