...
- **Slow first boot again, and your monthly distances will change.** A phone that replays a stale position after landing could bill a trip as dozens of instant intercontinental flights, so bursts like that are now flagged and any leg faster than 1200 km/h stops counting towards distance. Every history is re-checked in the background after the upgrade and the affected tracks and stats are rebuilt, so expect high CPU for a while and past monthly totals to move. A new index over your points is built during the upgrade as well, so give the loading page time and please do not restart the add-on while it is up. The background worker now waits for all of that to finish before it starts, so a busy disk cannot make one of the database changes time out and leave the app restarting in a loop. If an earlier attempt did get interrupted, the half-built statistics index it left behind is found and rebuilt on the way through. Your points, trips and imports are untouched
- Cities where your phone reported infrequently are counted again, so a home that had dropped out of your statistics comes back and time spent will generally go up. **Your existing months now repair themselves**, a few at a time in the background, instead of waiting until you press "Update stats" on the Stats page. Points in the United States, and a handful of other countries whose name the geocoder spells differently, were quietly missing from the countries you have visited and are repaired the same way. The "Max Gap Between Points" map setting is retired, its job now runs the same way for everyone. Importing got a round of fixes too, for GPX files with no elevation column, Polarsteps exports and the altitudes in Google Timeline files, and map labels in Hebrew and Arabic now read right to left. No app config changes required
